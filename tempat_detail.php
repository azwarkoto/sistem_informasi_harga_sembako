...
<?php
$row = $db->get_row("SELECT * FROM tb_kios WHERE id_kios='$_GET[ID]'");
?>
...
<div class="row">
    <div class="col-md-6">
        ...
        <h3>Nama Nama Sembako</h3>
        <div class="row">
            <?php
            $rows = $db->get_results("SELECT tb_harga.*, tb_sembako.nama_sembako,  tb_sembako.satuan, tb_sembako.deskripsi, tb_kios.nama_kios, tb_sembako.gambar            
             FROM tb_harga
             INNER JOIN tb_kios ON tb_kios.id_kios=tb_harga.id_kios
             INNER JOIN tb_sembako ON tb_sembako.id_sembako=tb_harga.id_sembako
             WHERE tb_kios.id_kios='$_GET[ID]'");
            foreach ($rows as $r) : ?>
                <div class="col-lg-3 col-md-4 col-xs-6 thumb">
                    <a class="thumbnail" href="#" data-image-id="" data-toggle="modal" data-title="<?= $r->nama_sembako ?>" data-caption="<?= strip_tags($r->deskripsi) ?> harga :<?= $r->harga ?>/ <?= $r->satuan ?>" data-image="assets/images/galeri/<?= $r->gambar ?>" data-target="#image-gallery">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title text-white" id="image-gallery-title"></h4>
            </div>
